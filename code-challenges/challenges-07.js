...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0 ; i < arr.length ; i++){
         ...
         if (arr[i].yearsOfExperience > 1){
              if((arr[i].firstName !== null && arr[i].firstName !== '') || (arr[i].lastName !== null && arr[i].lastName !== '') ){
                   if (arr[i].tech == 'JS'){
                        stObj.javaScript_devs = (countJS = countJS + 1)
                   }
                   else if (arr[i].tech === '.Net'){
                        stObj.dotNet_devs = (countDn = countDn + 1)
                   }
                   else if (arr[i].tech === 'Python'){
                        stObj.python_devs = (countPy = countPy + 1)
                   }
                   else if (arr[i].tech === 'Java'){
                        stObj.java_devs = (countJ  = countJ+ 1)
                   }
              }
              else{
                   stObj.rejectedApplicants = (reject = reject + 1);
              }
         }
         else{
              stObj.rejectedApplicants = (reject = reject + 1);
        ...
        stObj.totalApplicants =(total = total + 1)
    }
    return stObj ;
};
// -------------------------------------------------------------------------------------------------------
...
const classesAvg = (data) => {
    ...
    for ( let i = 0 ; i < data.grades.length ; i++ ){
         ...
         for(let y = 0 ; y <data.grades[i].classes.length ; y ++ ){
         
              for(let x = 0 ; x < data.grades[i].classes[y].classScores.length ; x++){
                    sum += data.grades[i].classes[y].classScores[x]
              }
         
         data.grades[i].classes[y].avg = Math.floor(sum / data.grades[i].classes[y].classScores.length)
         arrSum.push(sum)
         sum = 0
         }
    }
